...
/*@
    lemma lemma_count{L}: 
        \forall Graph *g, integer f, t, m;
            (0 <= (m - 1) <= g->ecnt) ==> (count(g,f, t, 0, m) == count(g, f, t, 0, (m - 1)) + count(g, f, t, (m - 1), m));

    lemma lemma_count_zero{L}:
        \forall Graph *g, integer f, t, m, n; 
            (m >= n) ==> (count(g, f, t, m, n) == 0);

    lemma lemma_count_one{L}:
        \forall Graph *g, integer f, t, m; 
            count_one_p(g, f, t, m) && (count(g, f, t, m, (m + 1)) == (g->edges[m].existent != 0 && g->edges[m].from == f && g->edges[m].to == t ? 1 : 0));

    lemma lemma1_count_split{L}: 
        \forall Graph *g, integer f, t, m, n, k; 
            (m <= n <= k) ==> count_split_p(g, f, t, m, n, k) && (count(g, f, t, m, k) == count(g, f, t, m, n) + count(g, f, t, n, k));

    lemma lemma2_count_split{L}:
        \forall Graph *g, integer f, t, m, n; 
            (m <= n) ==> count(g, f, t, m, (n + 1)) == count(g, f, t, m, n) + count(g, f, t, n, (n + 1));  

    lemma lemma_count_double_split{L}:
        \forall Graph *g, integer f, t, m, n, k; 
            (m <= k <= n - 1) ==> (count(g, f, t, m, n) == count(g, f, t, m, k) + count(g, f, t, k, (k + 1)) + count(g, f, t, (k + 1), n));

    lemma lemma_zero_after_ecnt{L}: 
        \forall Graph *g, integer f, t, m;
            graph_valid(g) ==> (g->ecnt <= (m - 1) < g->esize) ==> (count(g, f, t, (m - 1), m) == 0) && (g->edges[m - 1].existent == 0);

    lemma lemma_all_count{L}: 
        \forall Graph *g, integer k, f, t;
            (0 <= g->ecnt <= g->esize) ==> (all_count(g, f, t) == count(g, f, t, 0, g->esize)) &&
...
add_edge(Graph *g, int f, int t)
{
        ...
        */
        /*@
            assert \forall integer k; 
                0 <= k < g->esize ==> \forall integer f2, t2;
                    (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, k, k + 1) == count{Pre}(g, f2, t2, k, k + 1);
        */
        /*@
            assert \forall integer k; 
                0 <= k < g->esize && (k != g->ecnt -1) ==> \forall integer f2, t2;
                    (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, k, k + 1) == count{Pre}(g, f2, t2, k, k + 1);
        */
        ...
        /*@ ghost
            ...
            /@
                ...
                loop invariant \forall integer j;
                    (0 <= j < i) && (j != g->ecnt - 1) ==> \at(g->edges[j], Pre) == \at(g->edges[j], Here);

                loop invariant \forall integer j;
                    (0 <= j < i) && (j != g->ecnt - 1) ==> count{Pre}(g, f, t, j, (j + 1)) == count{Here}(g, f, t, j, (j + 1)); 
                ...
                loop invariant (i < g->ecnt - 1) ==> count{Pre}(g,f,t, 0, i+1) == count{Pre}(g, f, t, 0, i) + count{Pre}(g, f, t, i, i + 1);

                loop invariant (i >= g->ecnt - 1) ==> count{Pre}(g,f,t, 0, i+1) == count{Pre}(g, f, t, 0, i) + count{Pre}(g, f, t, i, i + 1);

                loop invariant (i >= g->ecnt - 1) ==> count{Here}(g,f,t, 0, i+1) == count{Here}(g, f, t, 0, i) + count{Here}(g, f, t, i, i + 1);

                loop invariant \forall integer j;
                    (0 <= j <= i) ==> \forall integer f2, t2;
                        (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, j) == count{Pre}(g, f2, t2, 0, j);

                loop invariant \forall integer j;
                    (0 <= j <= i - 1) ==> \forall integer f2, t2;
                        (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, j, j + 1) == count{Pre}(g, f2, t2, j, j + 1);

                loop invariant \forall integer j;
                    (0 <= j <= i - 1) ==> \forall integer f2, t2;
                        (f2 != f || t2 != t) ==> count{Pre}(g, f2, t2, 0, j + 1) == count{Pre}(g, f2, t2, 0, j) + count{Pre}(g, f2, t2, j, j + 1);

                loop invariant \forall integer j;
                    (0 <= j <= i - 1) ==> \forall integer f2, t2;
                        (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, j + 1) == count{Here}(g, f2, t2, 0, j) + count{Here}(g, f2, t2, j, j + 1);

                loop invariant \forall integer j;
                    (0 <= j && j >= i - 1 && j < g->esize) ==> \forall integer f2, t2;
                        (f2 != f || t2 != t) ==> count{Pre}(g, f2, t2, 0, j + 1) == count{Pre}(g, f2, t2, 0, j) + count{Pre}(g, f2, t2, j, j + 1);
    ...
    /*@ 
        ...
        loop invariant \forall integer f2, t2; 
            (f2 != f || t2 != t) ==> all_count(g, f2, t2) == all_count{Pre}(g, f2, t2);
        ...
        if (!g->edges[i].existent) {
            ...
            /*@ ghost
                ...
                /@
                    ...
                    loop invariant \forall integer j;
                        (0 <= j < l) && (j != i) ==> \at(g->edges[j], Pre1) == \at(g->edges[j], Here);

                    loop invariant \forall integer j;
                        (0 <= j < l) && (j != i) ==> count{Pre1}(g, f, t, j, (j + 1)) == count{Here}(g, f, t, j, (j + 1)); 

                    loop invariant (l <= i) ==> count{Pre1}(g, f, t, 0, l) == count{Here}(g, f, t, 0, l); 

                    loop invariant (l > i) ==> count{Here}(g, f, t, 0, l) == (count{Pre1}(g, f, t, 0, l) + 1);
                    
                    loop invariant (l <= i - 1) ==> count{Pre1}(g,f,t, 0, l + 1) == count{Pre1}(g, f, t, 0, l) + count{Pre1}(g, f, t, l, l + 1);

                    loop invariant (l <= i - 1) ==> count{Here}(g,f,t, 0, l + 1) == count{Here}(g, f, t, 0, l) + count{Here}(g, f, t, l, l + 1);
                    ...
                    loop invariant \forall integer j;
                        (0 <= j <= l) ==> \forall integer f2, t2;
                            (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, 0, j) == count{Pre1}(g, f2, t2, 0, j);

                    loop invariant \forall integer j;
                        (0 <= j <= l - 1) ==> \forall integer f2, t2;
                            (f2 != f || t2 != t) ==> count{Here}(g, f2, t2, j, j + 1) == count{Pre1}(g, f2, t2, j, j + 1);
                    ...
                    loop invariant \forall integer j;
                        (0 <= j <= l - 1) ==> \forall integer f2, t2;
                            (f2 != f || t2 != t) ==> count{Pre1}(g, f2, t2, 0, j + 1) == count{Pre1}(g, f2, t2, 0, j) + count{Pre1}(g, f2, t2, j, j + 1);
                
                    loop invariant \forall integer j;
                        (0 <= j && j >= l - 1 && j < g->esize) ==> \forall integer f2, t2;
                            (f2 != f || t2 != t) ==> count{Pre1}(g, f2, t2, 0, j + 1) == count{Pre1}(g, f2, t2, 0, j) + count{Pre1}(g, f2, t2, j, j + 1);
